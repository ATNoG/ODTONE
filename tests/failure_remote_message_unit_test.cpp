 ...
 * failure_unit_test: has received a "MIH_Capability_Discover.request"
 * failure_unit_test: Send MIH_Capability_Discover.request
 * failure_unit_test: has received a "MIH_Capability_Discover.confirm"
 * failure_unit_test: Send MIH_Event_Subscribe.request
 * failure_unit_test: has received a "MIH_Event_Subscribe.request"
 * failure_unit_test: has received a "MIH_Event_Subscribe.confirm"
 * failure_unit_test: Send MIH_Link_Up.indication
 * failure_unit_test: Send MIH_Link_Down.indication
 * failure_unit_test: Send MIH_Event_Unsubscribe.request
 * failure_unit_test: has received a "MIH_Event_Unsubscribe.request"
 * failure_unit_test: has received a "MIH_Event_Unsubscribe.confirm"
 * failure_unit_test: Send MIH_Link_Up.indication
 * failure_unit_test: Send MIH_Link_Down.indication
 * failure_unit_test: Send MIH_Link_Get_Parameters.request
 * failure_unit_test: has received a "MIH_Link_Get_Parameters.request"
 * failure_unit_test: has received a "MIH_Link_Get_Parameters.confirm"
 * failure_unit_test: Send MIH_Link_Actions.request
 * failure_unit_test: has received a "MIH_Link_Actions.request"
 * failure_unit_test: has received a "MIH_Link_Actions.confirm"
 * failure_unit_test: Send MIH_Link_Actions.request with PoA
 * failure_unit_test: has received a "MIH_Link_Actions.request"
 * failure_unit_test: has PoA TLV!
 * failure_unit_test: has received a "MIH_Link_Actions.confirm"
 * failure_unit_test: Send MIH_Link_Configure_Thresholds.request
 * failure_unit_test: has received a "MIH_Link_Configure_Thresholds.request"
 * failure_unit_test: has received a "MIH_Link_Configure_Thresholds.confirm"
 */
...
odtone::logger log_("failure_unit_test", std::cout);
	...
	: _mihf(io, cfg, boost::bind(&message_unit_test::event_handler, this, _1, _2))
{
	...
	link_message << mih::indication(mih::indication::link_register, mih::id("mihf2"))
	    & mih::tlv_interface_type_addr(lid);
	_mihf.async_send_mihf2(link_message);
...
void message_unit_test::event_handler(mih::message& msg, const boost::system::error_code& ec)
{
	...
	switch (msg.mid()) {
		...
		case mih::request::capability_discover: {
			...
			if(msg.source().to_string().compare("mihf2") != 0) {
				log_(0, "Error!");
				return;
			}
			...
			rsp << mih::confirm(mih::confirm::capability_discover, mih::id("mihf2"))
				& mih::tlv_status(mih::status_failure);
			rsp.tid(msg.tid());

			_mihf.async_send_mihf2(rsp);
	} break;

	case mih::confirm::capability_discover: {
		log_(0, "has received a \"MIH_Capability_Discover.confirm\"");

			if(msg.source().to_string().compare("mihf2") != 0 || msg.tid() != 55) {
				log_(0, "Error!");
				return;
			}
			...
			if(capabilities_event_list.is_initialized()) {
				if(capabilities_event_list.get().get(mih::mih_evt_link_up) == 1) {
					log_(0, "Error!");
					return;
				}
			}
	...
	case mih::request::event_subscribe: {
		...
		rsp << mih::confirm(mih::confirm::event_subscribe, mih::id("mihf2"))
			& mih::tlv_status(mih::status_failure);
		rsp.tid(msg.tid());
	...
	case mih::confirm::event_subscribe: {
		...
		if(msg.source().to_string().compare("mihf2") != 0 || msg.tid() != 126) {
			log_(0, "Error!");
			return;
		}
	...
	case mih::request::event_unsubscribe: {
		...
		rsp << mih::confirm(mih::confirm::event_unsubscribe, mih::id("mihf2"))
			& mih::tlv_status(mih::status_failure);
		rsp.tid(msg.tid());
	...
	case mih::confirm::event_unsubscribe: {
		...
		if(msg.source().to_string().compare("mihf2") != 0 || msg.tid() != 219) {
			log_(0, "Error!");
			return;
		}
		...
		// The response is independent of the Link SAP
		// Usually the MIHF always unsubscribe the events
//		if(st != mih::status_failure) {
//			log_(0, "Error!");
//			return;
//		}
	...
	case mih::request::link_get_parameters: {
		...
		rsp << mih::confirm(mih::confirm::link_get_parameters, mih::id("mihf2"))
			& mih::tlv_status(mih::status_failure);
		rsp.tid(msg.tid());
	...
	case mih::confirm::link_get_parameters: {
		...
		if(msg.source().to_string().compare("mihf2") != 0 || msg.tid() != 321) {
			log_(0, "Error!");
			return;
		}
	...
	case mih::request::link_actions: {
		...
		rsp << mih::confirm(mih::confirm::link_actions, mih::id("mihf2"))
			& mih::tlv_status(mih::status_failure);
		rsp.tid(msg.tid());
	...
	case mih::request::link_configure_thresholds: {
		...
		rsp << mih::confirm(mih::confirm::link_configure_thresholds, mih::id("mihf2"))
			& mih::tlv_status(mih::status_failure);
		rsp.tid(msg.tid());
	...
	case mih::confirm::link_configure_thresholds: {
		...
		if(msg.source().to_string().compare("mihf2") != 0 || msg.tid() != 999) {
			log_(0, "Error!");
			return;
		}
...
void message_unit_test::send_capability_discover()
{
	...
	req << mih::request(mih::request::capability_discover, mih::id("mihf2"));
	req.tid(55);
...
void message_unit_test::send_event_subscribe()
{
	...
	req << mih::request(mih::request::event_subscribe, mih::id("mihf2"))
		& mih::tlv_link_identifier(lid)
		& mih::tlv_event_list(event_list);
	req.tid(126);
...
void message_unit_test::send_event_unsubscribe()
{
	...
	req << mih::request(mih::request::event_unsubscribe, mih::id("mihf2"))
		& mih::tlv_link_identifier(lid)
		& mih::tlv_event_list(event_list);
	req.tid(219);
...
void message_unit_test::send_link_get_parameters()
{
	...
	req << mih::request(mih::request::link_get_parameters, mih::id("mihf2"))
...
void message_unit_test::send_link_actions()
{
	...
	req << mih::request(mih::request::link_actions, mih::id("mihf2"))
...
void message_unit_test::send_link_configure_thresholds()
{
	...
	req << mih::request(mih::request::link_configure_thresholds, mih::id("mihf2"))
		& mih::tlv_link_identifier(lid)
		& mih::tlv_link_cfg_param_list(lcpl);
	req.tid(999);
...
void message_unit_test::send_link_up()
{
	...
	req << mih::indication(mih::indication::link_up, mih::id("mihf2"))
		& mih::tlv_link_identifier(lid)
...
void message_unit_test::send_link_down()
{
	...
	req << mih::indication(mih::indication::link_down, mih::id("mihf2"))
		& mih::tlv_link_identifier(lid)
